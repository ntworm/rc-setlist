...
function stop(message) {
  ...
  console.error(
    '[ableton-deps] ABLETON_SDK_TGZ and ABLETON_CLI_TGZ must be existing absolute local paths.',
  );
  process.exit(2);
}
...
const result = spawnSync(
  ...
  npmExecPath ? [npmExecPath, ...npmArguments] : npmArguments,
  {
    cwd: process.cwd(),
    encoding: 'utf8',
    stdio: 'inherit',
...
console.log(
  '[ableton-deps] Authorized SDK and CLI installed locally without changing the public lockfile.',
);
