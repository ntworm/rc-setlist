...
test('the palette has sixteen colours and none of them is the RC accent', () => {
  ...
  assert.equal(editor.PALETTE_HEXES.includes('#ffa133'), false, 'orange is RC focus, not a song colour');
});
...
test('the original eight colours are still in the palette', () => {
  ...
  for (const hex of ['#d9c7a7', '#d6a89a', '#d9a3b0', '#bfa8d1', '#9db8d4', '#98c4c0', '#a9c4a0', '#c9c9c9']) {
    assert.equal(editor.PALETTE_HEXES.includes(hex), true, `${hex} was already assignable to a song`);
  }
});
...
test('loop reads as off, infinite, or a count', () => {
  assert.equal(editor.readForm(fakeForm({ name: 'A', 'loop-mode': 'off' }), 'song').loopCount, null);
  assert.equal(editor.readForm(fakeForm({ name: 'A', 'loop-mode': 'infinite' }), 'song').loopCount, -1);
  assert.equal(
    editor.readForm(fakeForm({ name: 'A', 'loop-mode': 'count', 'loop-times': '4' }), 'song').loopCount,
    4,
  );
  assert.equal(
...
test('a tag the panel does not show survives the save', () => {
  ...
  const out = editor.buildLocatorName('INTRO', read, 'song', 'INTRO [loop 4x] [click off] [bpm 136]');
...
test('a field the panel does show is rewritten, not duplicated', () => {
  ...
  assert.equal((out.match(/\[bpm/g) || []).length, 1, 'the old tempo must not linger beside the new one');
});
...
test('the section panel rewrites loop and click, and carries a song-only tag', () => {
  const read = editor.readForm(
    fakeForm({ name: 'VERSO I', bpm: '107', 'loop-mode': 'count', 'loop-times': '2', click: 'off' }),
    'section',
  );
  const out = editor.buildLocatorName('JÚLIA > VERSO I', read, 'section', 'JÚLIA > VERSO I [loop] [hidden]');
...
test('a relative section keeps its bare prefix instead of gaining a song title', () => {
  ...
  assert.equal(editor.buildLocatorName(head, read, 'section', '> VERSO I [bpm 107]'), '> VERSO I [bpm 107]');
});
...
test('reordering tags is not a change: a no-op save must not trigger a rename', () => {
  ...
  assert.equal(editor.isSameLocatorName('INTRO [bpm 136] [click]', 'INTRO [click] [bpm 136]'), true);
