...
**Applies to**: Ableton RC Setlist (`rc-setlist`)  

---

## 1. Executive Summary

Ableton RC Setlist computes song and setlist durations to provide musicians with exact stage timings. However, when an Ableton Live set contains arrangement master tempo automations (varying BPM across songs or sections) without explicit bracketed tags (`[bpm N]`), the extension cannot read the automation envelope.
