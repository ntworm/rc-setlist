...
class Manager(ControlSurface):
    def __init__(self, c_instance):
        ...
        try:
            ...
            self.show_message("%s %s: listening for OSC on port %d" % (abletonosc.RCBRIDGE_NAME, abletonosc.RCBRIDGE_VERSION, abletonosc.OSC_LISTEN_PORT))
            logger.warning("Started %s %s on address %s" % (abletonosc.RCBRIDGE_NAME, abletonosc.RCBRIDGE_VERSION, str(self.osc_server._local_addr)))
        except OSError as msg:
            self.show_message("%s: couldn't bind to port %d (%s)" % (abletonosc.RCBRIDGE_NAME, abletonosc.OSC_LISTEN_PORT, msg))
    ...
    def init_api(self):
        ...
        def version_callback(params):
            #--------------------------------------------------------------------------------
            # RC Bridge: lets a client tell this fork from a stock AbletonOSC before it
            # decides where replies will arrive.
            #--------------------------------------------------------------------------------
            return (abletonosc.RCBRIDGE_NAME, abletonosc.RCBRIDGE_VERSION, abletonosc.RCBRIDGE_UPSTREAM)
