"""
RC Bridge routing tests. No Live required: Live's modules are stubbed so the
package imports, and the OSC server is exercised over real loopback sockets.
...
def _stub_live_modules():
    for name in ("Live", "_Framework", "_Framework.EncoderElement", "ableton", "ableton.v2",
                 "ableton.v2.control_surface", "ableton.v2.control_surface.component"):
...
class TempoHandler(AbletonOSCHandler):
    ...
    def init_api(self):
        self.osc_server.add_handler("/live/song/get/tempo", lambda params: self._get_property(self.target, "tempo", params))
        self.osc_server.add_handler("/live/song/start_listen/tempo", lambda params: self._start_listen(self.target, "tempo", params))
        self.osc_server.add_handler("/live/song/stop_listen/tempo", lambda params: self._stop_listen(self.target, "tempo", params))
...
class RoutingTest(unittest.TestCase):
    ...
    def test_reply_goes_back_to_the_socket_that_asked(self):
        ...
        self.assertTrue(self.b.nothing_arrives(), "a bystander must not receive another client's reply")
    ...
    def test_listener_updates_reach_every_subscriber(self):
        ...
        self.pump()
        self.assertEqual(self.a.receive(), ("/live/song/get/tempo", (120.0,)), "the current value goes to the new subscriber")
        self.b.send(self.server, "/live/song/start_listen/tempo")
        self.pump()
        self.assertEqual(self.b.receive(), ("/live/song/get/tempo", (120.0,)))
        self.assertTrue(self.a.nothing_arrives(), "a second subscriber does not re-send the value to the first")
    ...
    def test_stop_listen_only_removes_the_requesting_client(self):
        ...
        self.pump()
        self.assertEqual(len(self.live.listeners), 1, "a is still subscribed, so Live keeps its listener")
