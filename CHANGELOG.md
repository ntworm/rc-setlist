...
- **Release artefact naming**: `RC-Setlist-1.0.0.ablx` and `RC-Setlist-1.0.0-Installation-Kit.zip` replace the `Ableton-RC-Setlist-<version>` convention.
...
- **Installer removes legacy 0.x package automatically**: `Install-RC-Bridge.ps1` (Windows) and `Install RC Bridge.command` (macOS) clean up pre-1.0 packages from legacy User Library / Extensions locations so Live only displays one RC Setlist entry in the Extensions menu.
