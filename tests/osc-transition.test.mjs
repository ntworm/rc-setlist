 ...
 */
test('OSC-only transport: an end-of-song [next] hands over to the next song at the marker', { timeout: 10_000 }, async (t) => {
  ...
  try {
    ...
    client.getCuePoints();
    await cuesLoaded;
    assert.deepEqual(bridgeState.manager.getState().songs.map((s) => s.title), ['MÚSICA 1', 'MÚSICA 2']);
    ...
    timers.push(setInterval(() => {
      if (mock['isPlaying']) mock['currentSongTime'] = Math.round((mock['currentSongTime'] + 0.2) * 100) / 100;
    }, 100));
    ...
    const state = bridgeState.manager.getState();
    assert.equal(state.activeSongIndex, 1, `never reached MÚSICA 2; positions: ${positions.join(' ')}`);
    const lastBeforeJump = positions.filter((p) => p < 68).at(-1);
    assert.ok(lastBeforeJump >= 64 && lastBeforeJump < 65, `crossed the marker at ${lastBeforeJump}`);
