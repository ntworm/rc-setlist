...
import { bridgeState, observePreRollPosition } from '../src/core/bridge-state.ts';
...
function installHarness({ cues, targetBeat = 32, isPlaying = false, metronome = false } = {}) {
  const saved = {
...
test('stopped Play sends temporary Click, one-bar position, then Play in order', async () => {
...
test('a second stopped Play replaces a pending pre-roll without losing temporary Click ownership', async () => {
  const harness = installHarness();
  try {
    registerOscListeners();
    await executeCommandAction(command('play'));
    harness.osc.emit('metronome', true);
    await executeCommandAction(command('play'));
...
test('set_pre_roll is local and broadcasts only server-owned state', async () => {
  ...
  try {
    ...
    assert.equal(harness.manager.getState().preRollEnabled, false);
...
test('manual Click during pre-roll prevents automatic restoration', async () => {
  const harness = installHarness();
  try {
    await executeCommandAction(command('play'));
    await executeCommandAction(command('metronome', { value: true }));
    observePreRollPosition(28);
    observePreRollPosition(32);

    assert.deepEqual(harness.osc.calls.filter(([kind]) => kind === 'metronome'), [
      ['metronome', true],
      ['metronome', true],
    ]);
  } finally {
...
test('manual Click changes observed directly from Live override automatic restoration', async () => {
  const harness = installHarness();
  try {
    registerOscListeners();
    await executeCommandAction(command('play'));
    harness.osc.emit('metronome', true); // expected acknowledgement of temporary Click-on
    harness.osc.emit('metronome', false); // operator changes Click in Live
    harness.osc.emit('metronome', true); // operator's final choice is Click-on
...
test('an observed rewind followed by another stopped sample cancels a Play that never started', async () => {
  const harness = installHarness();
  try {
    registerOscListeners();
    await executeCommandAction(command('play'));
