...
test('WebSocket profile and reliability core protocol', async () => {
  const testStorageDir = path.join(
    tmpdir(),
    'setlist-test-ws-' + Math.random().toString(36).substring(7),
  );
  ...
  try {
    ...
    assert.strictEqual(
      bridgeState.mcpClient,
      null,
      'isolated test mode must not connect to the live MCP bridge',
    );
    ...
    ws.send(
      JSON.stringify({ type: 'profile_create', name: newProfileName, commandId: 'create-p1' }),
    );
    ...
    const renamedProfilesState = await new Promise((resolve) => {
      const handler = (data) => {
        const msg = JSON.parse(data.toString());
        if (
          msg.type === 'profiles_state' &&
          msg.profiles.some((p) => p.name === renamedProfileName)
        ) {
          ws.off('message', handler);
          resolve(msg);
        }
    ...
    readOnlyWs.send(
      JSON.stringify({ type: 'sync_confirm', stateVersion: readOnlyAck.stateVersion }),
    );
    ...
    ws.send(
      JSON.stringify({
        type: 'save_lyrics',
        song: 'Song A',
        text: 'forbidden text update',
        commandId: 'cmd-save-lyrics-show',
      }),
    );
...
test('persisted commands reject stale completion after an active profile scope change', async () => {
  const testStorageDir = path.join(
    tmpdir(),
    `setlist-scope-race-${Math.random().toString(36).slice(2)}`,
  );
  fs.mkdirSync(testStorageDir, { recursive: true });
  ...
  try {
    ...
    assert.equal(
      await waitForPersistenceStart(reorderPersistence.started),
      originalPaths.customOrder,
    );
    ...
    assert.equal(
      events.some(({ type }) => type === 'state'),
      false,
    );
    assert.equal(
      events.some(({ message }) => /saved/i.test(message ?? '')),
      false,
    );
    ...
    assert.ok(
      (await waitForPersistenceStart(previewPersistence.started)).startsWith(originalPaths.audio),
    );
    await profileManager.select(otherProfile.id);
    ...
    assert.equal(
      events.some(({ message }) => /wrote preview|preview ready/i.test(message ?? '')),
      false,
    );
...
test('tracklist CSV exports active setlist, sections, automations and lyrics without placeholders', async () => {
  const testStorageDir = path.join(
    tmpdir(),
    `setlist-csv-details-${Math.random().toString(36).slice(2)}`,
  );
  fs.mkdirSync(testStorageDir, { recursive: true });
  setExtensionContext({ environment: { storageDirectory: testStorageDir } });
  const port = await getFreePort();
