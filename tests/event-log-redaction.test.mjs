...
test('EventLogger: Redacts secrets in key names (token, password, secret, key) and string values', async () => {
  ...
  try {
    ...
    assert.equal(
      content.includes(TOKEN_SENTINEL),
      false,
      'TOKEN_SENTINEL must NOT appear in log file!',
    );
    assert.equal(
      content.includes(PASSWORD_SENTINEL),
      false,
      'PASSWORD_SENTINEL must NOT appear in log file!',
    );
    assert.equal(
      content.includes('NESTED_SECRET_999'),
      false,
      'NESTED_SECRET_999 must NOT appear in log file!',
    );
