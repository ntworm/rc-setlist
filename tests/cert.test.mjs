...
import { certCoversRequiredAltNames, privateKeyMatchesCertificate } from '../src/server/cert.ts';

async function certificate({ altNames, notBeforeDate, notAfterDate } = {}) {
  return selfsigned.generate([{ name: 'commonName', value: 'rc-setlist.local' }], {
    algorithm: 'sha256',
    keyType: 'ec',
    curve: 'P-256',
    notBeforeDate,
...
test('certificate validation requires localhost, loopback, LAN addresses, and current validity', async () => {
  ...
  assert.equal(
    certCoversRequiredAltNames(valid.cert, ['192.168.1.10'], new Date('2027-01-01T00:00:00Z')),
    false,
  );
