...
function forbidProductionPorts(t) {
  ...
  t.mock.method(dgram.Socket.prototype, 'bind', function (port, ...args) {
    ...
    assert.ok(
      ![11000, 11001, 11020, 11021, 11101, 11201].includes(requested),
      `must not bind production port ${requested}`,
    );
...
test(
  'the client finds RC Bridge and talks to it over its own socket',
  { timeout: 5_000 },
  async (t) => {
    ...
    try {
      ...
      assert.ok(
        snapshot.oscListenPort > 0 && snapshot.oscListenPort !== 11001,
        'an ephemeral port of its own',
      );
      assert.equal(
        globalThis.abletonOSCSocket,
        undefined,
        'no socket is shared through globalThis in bridge mode',
      );
...
test(
  'without RC Bridge the client falls back to the stock AbletonOSC path',
  { timeout: 5_000 },
  async (t) => {
    forbidProductionPorts(t);
    const previousSocket = globalThis.abletonOSCSocket;
    ...
    try {
      ...
      assert.ok(
        ...
        'the probe waited for its timeout before falling back',
      );
      ...
      assert.ok(
        globalThis.abletonOSCSocket,
        'the legacy path shares its reply socket through globalThis',
      );
...
test(
  "'connect' fires only once the bridge socket can send, so the connect handler's requests go out",
  { timeout: 5_000 },
  async (t) => {
    ...
    try {
      ...
      assert.equal(
        bridgeAtConnect,
        'rcbridge',
        'the snapshot already names the bridge inside the connect handler',
      );
...
test(
  'stop() during the probe discards the answer instead of adopting a socket after shutdown',
  { timeout: 5_000 },
  async (t) => {
    forbidProductionPorts(t);
    const previousSocket = globalThis.abletonOSCSocket;
