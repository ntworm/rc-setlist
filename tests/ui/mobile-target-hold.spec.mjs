...
test('mobile song and section targets require hold and fire once while playing', async ({
  page,
}) => {
  ...
  await expect
    ...
    .toEqual([{ type: 'jump', songIndex: 1, sectionIndex: 1 }]);
});

test('mobile hold works while stopped, active targets stay inert, and drag cancels', async ({
  page,
}) => {
...
test('armed mobile song movement previews one insertion slot and commits the adjusted order', async ({
  page,
}) => {
  ...
  expect(await jumpMessages(page)).toEqual([]);
  await expect(
    page.locator('.song-item.is-reordering, .song-item.drop-before, .song-item.drop-after'),
  ).toHaveCount(0);
});

test('native Chrome touch hold then vertical drag previews and commits a song reorder', async ({
  page,
  browserName,
}) => {
...
test('direct-touch drag suppression restores draggable cards for cancellation, blur, and render reset', async ({
  page,
}) => {
  ...
  await begin(61);
  await source.dispatchEvent('pointerleave', {
    pointerType: 'touch',
    pointerId: 61,
    isPrimary: true,
  });
...
test('native Chrome render during a direct touch preserves compatibility-click suppression', async ({
  page,
  browserName,
}) => {
  test.skip(browserName !== 'chromium', 'CDP native touch input is Chromium-specific');
  ...
  for (const [setlistVersion, holdMs] of [
    [71, 100],
    [72, 550],
  ]) {
...
test('desktop native drag shares the insertion preview and reorder calculation', async ({
  page,
}) => {
...
test('native Chromium touch suppresses compatibility click and pans without a jump', async ({
  page,
  browserName,
}) => {
  test.skip(browserName !== 'chromium', 'CDP native touch input is Chromium-specific');
...
test('desktop double-click opens the marker panel when stopped and blocks it during performance', async ({
  page,
}) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  const fixture = await page.evaluate(async () =>
...
test('every tag the marker panel writes has a badge on the card', async ({ page }) => {
  ...
  await expect(
    page.locator('.section-btn[data-song="0"][data-section="0"] .skip-badge'),
  ).toHaveCount(1);
  await expect(
    page.locator('.section-btn[data-song="0"][data-section="1"] .click-badge'),
  ).toHaveText(/CLICK/);
});

test('a marker with a save still in flight cannot be reopened onto stale values', async ({
  page,
}) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  const fixture = await page.evaluate(async () =>
