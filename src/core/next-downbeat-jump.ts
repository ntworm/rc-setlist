 ...
 */
export function nextDownbeat(anchor: number, tempo: number, signatureNumerator: number): number {
 ...
 */
export function nextQuantizationBoundary(anchor: number, quantizationBeats: number): number {
...
export function getQuantizationBeats(
  ...
  switch (val) {
    ...
    case 4:
      return sig; // 1 Bar
    case 5:
      return 0.5 * sig; // 1/2 Bar
    case 6:
      return (1 / 3) * sig; // 1/2 Triplet
    case 7:
      return 1; // 1/4 Bar (1 Beat)
    case 8:
      return 2 / 3; // 1/4 Triplet
    case 9:
      return 0.5; // 1/8 Bar (0.5 Beat)
    case 10:
      return 1 / 3; // 1/8 Triplet
    case 11:
      return 0.25; // 1/16 Bar (0.25 Beat)
    case 12:
      return 1 / 6; // 1/16 Triplet
    case 13:
      return 0.125; // 1/32 Bar
    case 14:
      return 1 / 12; // 1/32 Triplet
    case 15:
      return 0.0625; // 1/64 Bar
    case 16:
      return 1 / 24; // 1/64 Triplet
    default:
      return sig; // default 1 bar
  }
}

/**
 * Manages the lifecycle and public surface of JumpScheduler.
 */
export class JumpScheduler {
  private pending: PendingJump | null = null;
  private listeners: Set<(event: { type: 'replaced' | 'executed'; pending: PendingJump }) => void> =
    new Set();
  ...
  on(
    listener: (event: { type: 'replaced' | 'executed'; pending: PendingJump }) => void,
  ): () => void {
  ...
  schedule(
    ...
    const quantBeats = getQuantizationBeats(
      state.clipTriggerQuantization,
      state.signatureNumerator,
    );
    ...
    const landingTime = immediate
      ? state.currentSongTime
      : nextQuantizationBoundary(state.currentSongTime, quantBeats);
  ...
  private emit(event: { type: 'replaced' | 'executed'; pending: PendingJump }): void {
    for (const l of this.listeners) {
      try {
        ...
        log.error('core', 'jump scheduler listener threw', {
